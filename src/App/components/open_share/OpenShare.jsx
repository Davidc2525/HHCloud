...
import { UploaderSelect} from "../explorer/UploaderSelect.jsx";
...
/**/
import OrderSelect from "../explorer/OrderSelect.jsx";
import {
	getParent,
	isRoot,
	parsePath,
...
//import SharedWithMe from "../share/SharedWithMe.jsx"
...
@connect(state=>{
	...
	return {auth,upload,online,filter:toolBar.get("filter"),path,currentType,selecteds,isSelecteMode:selection.get("isSelecteMode")}
})
class OpenShare extends React.Component{
	constructor(props){
		super(props);
		//window.ex = this
	...
	render() {
		const {classes,width,upload,currentType,auth,fullScreen}= this.props
		const stateLogin = auth.get("state");
		const position  = (currentType=="folder")?"fixed":"absolute"

		const uploadActive = upload.get("active")

		const showSharedWithMe = (stateLogin==STATES[1]&&!fullScreen)
		return (
			...
			<div id="OpenShare">
				...
				<Grid container>
					{showSharedWithMe&&
						<Grid item xs={4}>
							<div id="sharedWithMe">
								<SharedWithMe isEmbedd={true}/>
							</div>
						</Grid>	
					}

					<Grid item xs={showSharedWithMe?8:12} >
						<div className={showSharedWithMe?classes.dividerSharedAndViewExplorer:""}>
							<ViewExplorer/>
						</div>
					</Grid>	
				</Grid>
...
@connect(state=>{
	...
	var toolBar = state.getIn(["open_share","toolBar"]);
	var selection = state.getIn(["open_share","selection"]);
	var selecteds = state.getIn(["open_share","selection","selecteds"]);
	return {locationData,path,online,filter:toolBar.get("filter"),router,currentType,selecteds,isSelecteMode:selection.get("isSelecteMode")}
})
	...
	handleAction(event,data){
		...
		if (data.action == "goparent") {
			...
			if (!isRoot(( parsePath(this.props.router.location.hash))) ) {
				...
				locationData.hash = getParent(parsePath(locationData.hash))
				store.dispatch(push(locationData));
		...
		if (data.action == "refresh") {
			var locationData = this.props.locationData;
			store.dispatch(fetchingPath({
				...locationData,
				path: locationData.currentPathWithUser
			}));
		}
		...
		if (data.action == "downloadFile") {
			var locationData = this.props.locationData;
			download(locationData.owner, locationData.spath, locationData.subpath);
		}
